...
class AddItemToCart(BasePage):
    ...
    def is_search_field_displayed(self):
        self.logger.info('[LOGS] Czy wyszukiwarka strony jest widoczna')
    ...
    def checkout_placeholder_value(self):
        self.logger.info('[LOGS] Weryfikacja poprawnej wartości placeholdera')
    ...
    def click_on_search_input(self):
        self.logger.info('[LOGS] Kliknięcie w puste pole wyszukiwarki')
        search_field = self.driver.find_element(*SearchEngineLocators.SEARCH_FIELD)
        search_field.click()
    ...
    def type_proper_value(self, items_name):
        self.logger.info('[LOGS] Wpisana wartość w wyszukiwarce to {itemsname}'.format(itemsname=items_name))
        search_value = self.driver.find_element(*SearchEngineLocators.SEARCH_FIELD)
        search_value.send_keys(items_name)
    ...
    def is_submit_button_enabled(self):
        self.logger.info('[LOGS] Czy przycisk zatwierdzenia jest dostępny')
    ...
    def submit_typed_value(self):
        self.logger.info('[LOGS] Wyszukiwanie pożądanej wartości')
        submit_value = self.driver.find_element(*SearchEngineLocators.SEARCH_SUBMIT)
        submit_value.click()
    ...
    def checkout_searched_value(self):
        self.logger.info('[LOGS] Weryfikacji znalezionych produktów o podanej wartości')
        try:
    ...
    def select_available_item(self):
        self.logger.info('[LOGS] Wybranie danego produktu z listy')
    ...
    def is_continuation_popup_displayed(self):
        self.logger.info('[LOGS] Czy okno kontynuacji zakupu jest dostępne')
        wait = WebDriverWait(self.driver, 40)
    ...
    def go_to_cart(self):
        self.logger.info('[LOGS] Przejście do koszyka')
    ...
    def check_cart_value(self):
        self.logger.info('[LOGS] Weryfikacja ilości produktów dodanych do koszyka')
        wait = WebDriverWait(self.driver, 40)
